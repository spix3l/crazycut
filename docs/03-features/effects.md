...
- **FX-9** Built-in transform on every visual clip (not an added effect): position X/Y, scale %, rotation °, anchor point, opacity 0–100%, flip H/V, fit/fill/stretch framing mode for mismatched aspect.
  ...
  - Editable on the monitor as well as in the inspector. See TXT-6. `app/lib/modules/editor/domain/canvas_geometry.dart` mirrors `rasterizeLayer()` so the handles land on the pixels; `app/native_test/editor/canvas_gizmo_parity_integration_test.dart` renders through the engine at several canvas sizes and asserts the two still agree.
