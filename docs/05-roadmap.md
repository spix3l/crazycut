...
>
> **Packaging:** `tools/package-macos.sh` builds a release engine + app, embeds `libcrazycut.dylib`, `crazycut_worker`, and every third-party dylib (ffmpeg, whisper/ggml, all rewritten to `@rpath`) inside the bundle, audits that nothing references Homebrew or the build tree, and produces `dist/CrazyCut.dmg`. The bundle is self-contained and runs on a clean machine. Signing auto-detects a Developer ID Application identity and notarization takes a keychain profile or an App Store Connect API key; setup is documented in `docs/quality/macos-signing.md`.
>
> **Not built:** the signed Windows NSIS installer and the update check (there is no release feed to check against yet).
...
Custom LUT import · curves/wheels · bezier graph editor · volume automation lanes · audio FX (EQ/comp) · speed ramps/time remap · nested sequences · direct upload integrations · Linux support · HDR delivery.
...
Local transcription left the list the same way, pulled out by the shorts
workflow. It ships as half of `03-features/ai-assist.md` (**AI**) — the other
half being a vendor-neutral LLM provider layer — with
`03-features/shorts.md` (**SHT**) as its first consumer. Both amend
