...
| Area tracking | Implemented (macOS) | Region solve (OpenCV, static, `CC_WITH_TRACKING`), `trackers[]`, corner-pin warp in the shared preview/export compositor, canvas tool (menu bar + inspector) with a live region readout, one-action replace-with-image, Track tab with live progress, and a timeline confidence stripe. Deterministic and cancellable, with engine and Dart coverage including a known-motion fixture and Dart↔engine warp parity. Windows build and physical QA of the solver remain unverified |
