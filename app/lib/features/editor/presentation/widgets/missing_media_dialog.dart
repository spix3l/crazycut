...
import '../../../../state/media_relink.dart';
...
class _MissingMediaDialogState extends State<_MissingMediaDialog> {
  ...
  Future<void> _scan(List<String> paths) async {
    ...
    final plan = MediaRelinker.instance.plan(_missing, candidates);
    if (!mounted) return;
    setState(() {
      _plan = plan;
      _scanning = false;
      _status = '${candidates.length} file${candidates.length == 1 ? '' : 's'} '
      ...
      child: Row(
        children: [
          ...
          if (match != null)
            ...
            Text(size, style: CcType.style(size: 10, color: CcColors.textTertiary)),
        ],
      ),
    );
  }
