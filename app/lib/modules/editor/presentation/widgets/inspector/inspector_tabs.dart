      ...
      child: Column(
        ...
        children: [
          ...
          TimecodeRow(
            ...
            enabled: !locked,
            onSubmitted:
                (value) => controller.setClipTiming(clip.id, start: value),
          ),
          TimecodeRow(
            label: 'Duration',
            value: clip.duration,
            ...
            enabled: !locked,
            onSubmitted:
                (value) => controller.setClipTiming(clip.id, duration: value),
          ),
          TimecodeRow(
            label: 'End',
            value: clip.end,
            ...
            enabled: !locked,
            onSubmitted:
                (value) => controller.setClipTiming(
                  clip.id,
                  duration: value.minus(clip.start),
                ),
          ),
          TimecodeRow(
            label: 'Source in',
            ...
            enabled: !locked,
            onSubmitted:
                (value) => controller.setClipTiming(clip.id, sourceIn: value),
          ),
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                InfoRow('Speed', _speedControl(controller, clip, locked)),
                InfoRow('Track', _value(track?.name ?? 'None')),
                if (asset != null)
                  InfoRow(
                    'Available',
                    _value(Rt.toTimecode(asset.duration, fps)),
                  ),
                InfoRow('Handles', _value(_handles(asset, fps))),
                InfoRow(
                  'Linked',
                  _value(clip.linkedGroup == null ? 'no' : 'A/V'),
                ),
              ],
            ),
          ),
  ...
  static Widget _speedControl(
    ...
    final canChange =
        ...
        controller.doc
            ...
            .every(
              (candidate) =>
                  !(controller.doc.trackById(candidate.trackId)?.lock ?? false),
            );
    return CcTooltip(
      message:
          canChange
                      ...
                      ? () => showCcMenu(anchorContext, [
                        for (final preset in TimelineEdits.clipSpeedPresets)
                          CcMenuItem(
                            preset.$3,
                            checked:
                                (preset.$1 / preset.$2 - clip.speedValue)
                                    .abs() <
                                0.000001,
                            onTap:
                                () => controller.setClipSpeed(
                                  clip.id,
                                  preset.$1,
                                  preset.$2,
                                ),
