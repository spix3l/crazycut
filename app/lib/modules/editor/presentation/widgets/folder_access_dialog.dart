...
class _FolderAccessDialogState extends State<_FolderAccessDialog> {
  ...
  Future<void> _grant(FolderRequest request) async {
    ...
    await widget.controller.dependencies.sandbox.remember(picked);
    final ok =
        request.needsWrite
            ? widget.controller.dependencies.sandbox.canWrite(request.folder)
            : widget.controller.dependencies.sandbox.canRead(request.folder);
    if (!mounted) return;

    if (!ok) {
      setState(() {
        _status =
            'That folder doesn’t cover “${request.name}”. '
    ...
    return CcDialogShell(
      ...
      sections: [
        ...
        if (_status != null)
          Text(
            _status!,
            style: CcType.style(size: 11, color: CcColors.warning, height: 1.4),
          ),
