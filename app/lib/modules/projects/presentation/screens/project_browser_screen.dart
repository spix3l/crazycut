...
import 'package:crazycut_app/modules/projects/application/onboarding.dart';
...
import 'package:crazycut_app/modules/projects/presentation/widgets/project_card.dart';
import 'package:crazycut_app/modules/projects/presentation/widgets/project_skeleton.dart';
import 'package:crazycut_app/modules/projects/presentation/widgets/recovery_banner.dart';
import 'package:crazycut_app/modules/projects/presentation/widgets/welcome_panel.dart';
...
class _ProjectBrowserScreenState extends State<ProjectBrowserScreen> {
  ...
  Future<void> _openExistingProject() async {
    ...
    if (file == null) return;
    try {
      await _session.rememberProjectLocation(file.path);
      await _session.openPath(file.path);
  ...
  Future<void> _openSampleProject() async {
    ...
    await _session.openPath(file.path);
    await _dependencies.onboarding.showAgain();
    if (!mounted) return;
    await context.router.push(EditorRoute());
    await _reload();
  }
  ...
  Future<void> _rename(ProjectSummary summary, String path) async {
    ...
    await _session.noteRenamed(path, renamed.path);
    await _reload();
  }
  ...
  Future<void> _delete(ProjectSummary summary, String path) async {
    ...
    await _session.forgetRecent(path);
    await _reload();
  }
